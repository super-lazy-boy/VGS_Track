...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        - layers: 每个阶段的块数量列表，如[2,2,2,2]对应resnet18
        - num_classes: 分类类别数
        - zero_init_residual: 是否对残差分支最后一个BN层进行零初始化
        - groups: 分组卷积的组数 (用于ResNeXt)
        - width_per_group: 每组的通道宽度 (用于ResNeXt)
        - replace_stride_with_dilation: 是否用空洞卷积替代步长卷积
        - norm_layer: 归一化层类型
        """
